...
namespace machine {
	...
	uint32_t computer_split::retrieve_memory_address(const memory_operand& mem_op) const {
	...
	uint32_t computer_split::retrieve_operand_value(const operand_arg& op) const {
		switch (op.type) {
	...
	uint32_t computer_split::retrieve_result_value(const result_arg& res) const {
		switch (res.type) {
			case result_arg::type_t::REGISTER:
	...
	void computer_split::set_result_value(const result_arg& res, uint32_t value) {
		switch (res.type) {
			case result_arg::type_t::REGISTER:
	...
	void computer_split::execute_instruction(const instruction_t& instr) {
	...
	void computer_split::run(int max_steps) {
		if (max_steps <= 0) {
